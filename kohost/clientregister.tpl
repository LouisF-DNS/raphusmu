...
{if !$registrationDisabled}
    <div id="registration" class="tt-registration-form tt-custom-radius p-4 p-lg-5 bg-white">
        ...
        <div class="auth-footer mt-4">{$LANG.allReadyRegister}<br>
            You can <a href="{$WEB_ROOT}/clientarea.php"> {$LANG.login} </a> <span class="text-lowercase">{$LANG.orText}</span> <a href="{routePath('password-reset-begin')}">{$LANG.forgotpw}</a> your account password.</div>
    </div>
{/if}
